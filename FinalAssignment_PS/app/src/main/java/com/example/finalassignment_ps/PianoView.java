...
public class PianoView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < NB; i++) {
            ...
            RectF rectangle = new RectF(left, 0, right, h);
            whites.add(new Key(rectangle, i + 1));

            if (i != 0  &&   i != 3  &&  i != 7  &&  i != 10) {
                rectangle = new RectF((float) (i - 1) * keyWidth + 0.5f * keyWidth + 0.25f * keyWidth, 0,
                        (float) i * keyWidth + 0.25f * keyWidth, 0.67f * height);
                blacks.add(new Key(rectangle, count));
                count++;
            }
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        for (Key k : whites) {
            canvas.drawRect(k.rectangle, k.down ? red : white);
        }
        ...
        for (Key k : blacks) {
            canvas.drawRect(k.rectangle, k.down ? red : black);
        }
    }
